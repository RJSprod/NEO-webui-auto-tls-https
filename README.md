...
The one connection is kept for as long as the page is: Hypercorn's own habit of closing a connection after a thousand requests - sized for HTTP/1.1, where that is one request at a time - is switched off, because over HTTP/2 a page reaches a thousand within minutes of a Generate (progress polls, thumbnails, clicks), and closing the connection there took the page's heartbeat and queue streams down with it: the browser reported *Connection errored out.* while the image had in fact been generated.
